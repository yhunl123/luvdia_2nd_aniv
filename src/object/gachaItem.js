// 레어 등급 오브젝트
const item_r = [
    {
        id: 1,
        name: '러브리아의 편지 1',
        url: '/',
        data: '와아~ 2주년! 벌써 2주년!<br>' +
            '시간이 참 빨리 지나갔더라고요~!<br>' +
            '매번 항상 재미난 뱅송 감사하고!<br>' +
            '앞으로도 잘 부탁드려요!',
        type: 'text',
        grade: 'R',
        unlocked: false,
        isNew: false
    },
    {
        id: 2,
        name: '러브리아의 편지 2',
        url:'/',
        data: '러브다이아 2주년 정말 축하드립니다.<br>' +
            '앞으로 3주년 4주년 20주년 까지 잘 부탁드립니다!',
        type: 'text',
        grade: 'R',
        unlocked: false,
        isNew: false
    },
    {
        id: 3,
        name: '러브리아의 편지 3',
        url:'/',
        data: '오직 럽다만! 오직 럽다만!<br>' +
            '빙하유 양메이 유리리 사랑해!!<br>' +
            '뽀에버!!!',
        type: 'text',
        grade: 'R',
        unlocked: false,
        isNew: false
    },
    {
        id: 4,
        name: '러브리아의 편지 4',
        url:'/',
        data: '러브다이어와 함께 한 2년의 시간.<br>' +
            '너무나도 감사하고 소중한 시간입니다.<br>' +
            '앞으로도 요로시쿠네!',
        type: 'text',
        grade: 'R',
        unlocked: false,
        isNew: false
    },
    {
        id: 5,
        name: '러브리아의 편지 5',
        url:'/',
        data: '2주년 축하드려요!!<br>' +
            '벌써 2주년이라니 얼마전이 1주년 이였던것 같은데 시간이 정말 빠르게 느껴지네요.<br>' +
            '최근에는 자주 보지 못했지만 럽다를 좋아하는 마음은 변하지않고 응원하고 있어요!<br>' +
            '럽다 2주년 정말 축하드리고 앞으로도 계속 잘부탁드려요!!',
        type: 'text',
        grade: 'R',
        unlocked: false,
        isNew: false
    },
    {
        id: 6,
        name: '러브리아의 편지 6',
        url:'/',
        data: '하유님을 만나 세상의 빛이 다시 컬러플하게 바뀌었습니다.<br>' +
            '사랑합니다 하유님!',
        type: 'text',
        grade: 'R',
        unlocked: false,
        isNew: false
    },
    {
        id: 7,
        name: '러브리아의 편지 7',
        url:'/',
        data: '앞으로도 즐겁게! 잘부탁해오!',
        type: 'text',
        grade: 'R',
        unlocked: false,
        isNew: false
    },
    {
        id: 8,
        name: '러브리아의 편지 8',
        url:'/',
        data: '와! 벌써 2주년이래요!<br>' +
            '데뷔 뱅송 때 부터 못봤지만...!<br>' +
            '그래도 언제나 재미난 뱅송 해주셔서 감사합니다!<br>' +
            '<br>' +
            '앞으로도 잘 부탁드려요!<br>' +
            '러브다이아! 채고다!<br>' +
            '월.클.러.브.다.이.아.홧.팅!',
        type: 'text',
        grade: 'R',
        unlocked: false,
        isNew: false
    },
    {
        id: 9,
        name: '러브리아의 편지 9',
        url:'/',
        data: '유입된지 얼마 안되서 즐거운 덕질 중입니다!<br>' +
            '덕분에 팍팍한 세상에서 매일 즐겁게 웃을 수 있어요.<br>' +
            '럽다 2주년 너무 축하드리고 늘 감사합니당!',
        type: 'text',
        grade: 'R',
        unlocked: false,
        isNew: false
    },
    {
        id: 10,
        name: '러브리아의 편지 10',
        url:'/',
        data: '러브다이아! 2주년 추카드러요!!!<br>' +
            '앞으로도 아프지 말구 행복하구 좋은일만있으시구 꽃길만 걷길바라며~<br>' +
            '3주년 가보쟈~~~~!!!',
        type: 'text',
        grade: 'R',
        unlocked: false,
        isNew: false
    },
    {
        id: 11,
        name: '러브리아의 편지 11',
        url:'/',
        data: '우리가 만난지 벌써 2년 그 사이에 정말 많은 일들이 있었지만<br>' +
            '잘 버텨준 우리 러브다이아 맴버들 너무 고맙고~ 사랑하고<br>' +
            '우리 20년 200년까지 쭉 같이 달려갑시다!<br>' +
            '사랑해~♡',
        type: 'text',
        grade: 'R',
        unlocked: false,
        isNew: false
    },
    {
        id: 12,
        name: '러브리아의 편지 12',
        url:'/',
        data: '벌써 2년이 다되어가다니 새삼 시간이 많이 흘렀다는게 느껴지네 ㅎㅎㅎ<br>' +
            '항상 노력하고 발전하는 럽다 원하는걸 꼭 이루고 누가 보더라도 "아 그룹? 알아 유명하자나" 이렇게 얘기가 나왔으면 좋겠다 ㅎㅎ',
        type: 'text',
        grade: 'R',
        unlocked: false,
        isNew: false
    },
    {
        id: 13,
        name: '러브리아의 편지 13',
        url:'/',
        data: '벌써 2주년이라니... 시간 참 빠르네요. 하유님 사... 사.... 사.... 친구를 만나느라 사사사!<br>' +
            '암튼 제 사이오시 하유님 화이팅! 메이메이, 곤듀님 화이팅! 모두모두 응원해요!!!!!!',
        type: 'text',
        grade: 'R',
        unlocked: false,
        isNew: false
    },
    {
        id: 14,
        name: '러브리아의 편지 14',
        url:'/',
        data: '러브다이아2주년 너무 축하드립니다.<br>' +
            '앞으로 쭉쭉 꽃길만 걸으세요',
        type: 'text',
        grade: 'R',
        unlocked: false,
        isNew: false
    },
    {
        id: 15,
        name: '러브리아의 편지 15',
        url:'/',
        data: '어느덧 리리를 본지 644일이 됐어! 앞으로도 리리와 재밌는 추억 만들면서 놀고 싶어!<br>' +
            '2주년 축하해!!',
        type: 'text',
        grade: 'R',
        unlocked: false,
        isNew: false
    },
    {
        id: 16,
        name: '러브리아의 편지 16',
        url:'/',
        data: '유튜브 부터 최소 2년이상 봐왔는대 벌써 시간이 이렇게 가고있네요.<br>' +
            '아쉬운 부분도 있엇지만 남은 3인 밝은모습을 기대하며 괴롭고 힘들어도<br>' +
            '반드시 극복하겠다는 마인드로 항상파이팅 하세요.<br>' +
            '카페에서나 엑스(트위치)지겨보고 응원하고 있습니다',
        type: 'text',
        grade: 'R',
        unlocked: false,
        isNew: false
    },
    {
        id: 17,
        name: '러브리아의 편지 17',
        url:'/',
        data: '럽다 벌써 2주년~!!!<br>' +
            '항상 응원합니다<br>' +
            '사랑해용!!<br>' +
            '러브다이아!',
        type: 'text',
        grade: 'R',
        unlocked: false,
        isNew: false
    },
    {
        id: 18,
        name: '러브리아의 편지 18',
        url:'/',
        data: '많은 시간이 흘러 벌써 2주년이네요<br>' +
            '데뷔하고 1주년이 바로 엊그제 같은데<br>' +
            '벌써 저희가 만난 인연이 2년이나 됐네요<br>' +
            '여러가지 좋은일도 많고 탈도 많았지만<br>' +
            '지금까지 계속 함께 해주셔서 정말 감사해요<br>' +
            '러브다이아로 인해 제 인생이 문자그대로 180°바뀌었어요<br>' +
            '하지 않던 일도 많이 하게되었고<br>' +
            '잊지못할 경험도 많이 하게되었어요<br>' +
            '러브다이아는 이제 제 인생에 없어서는 안될 중요한 그룹이에요<br>' +
            '앞으로도 잘부탁드리고 앞으로도 열심히 방송하고<br>' +
            '시청자들과 즐거운 추억을 쌓으셨으면 좋겠어요<br>' +
            '2주년 정말 축하드려요!!',
        type: 'text',
        grade: 'R',
        unlocked: false,
        isNew: false
    },
    {
        id: 19,
        name: '러브리아의 편지 19',
        url:'/',
        data: '벌써 2주년이라고 하니까 시간이 정말 빠르게 흘러가는것 같아요!<br>' +
            '앞으로도 잘 부탁드리고!<br>' +
            '인생에서 가장 기억에 남는 버튜버 그룹이 있다면 누구냐는 질문에는<br>' +
            '꼭 러브다이아 라는 그룹이 기억에 남고 제일 좋아하는 그룹이라고 말하고 다닐겁니다!<br>' +
            '러브다이아 2주년 정말 축하드리고 꼭 행복으로 가득한 일만 있었으면 좋겠습니다!',
        type: 'text',
        grade: 'R',
        unlocked: false,
        isNew: false
    },
    {
        id: 20,
        name: '러브리아의 편지 20',
        url:'/',
        data: '러브다이아! 2주년!<br>' +
            '러브다이아 항상<br>' +
            '재밌게 보고있어요!<br>' +
            '앞으로도 모두들 힘내시고<br>' +
            '앞으로의 성공을 기원하며<br>' +
            '러브다이아 화이팅!<br>' +
            '빙하유! 양메이! 유리리!<br>' +
            '나머지 분들도 화이팅!',
        type: 'text',
        grade: 'R',
        unlocked: false,
        isNew: false
    },
    {
        id: 21,
        name: '러브리아의 편지 21',
        url:'/',
        data: '작년에 버츄얼방송을 러브다이아로 처음 보기 시작해서 지금까지 계속 봐왔는데 지금까지 감사했고 앞으로도 잘 부탁드려요!<br>' +
            '<br>' +
            '저희 러브리아들 항상 먼저 챙겨주시는것도 감사하지만 언제나 건강하게 방송해 주세요!<br>' +
            '<br>' +
            '언제나 어디서나 응원합니다!<br>' +
            '럽다 화이팅!!!',
        type: 'text',
        grade: 'R',
        unlocked: false,
        isNew: false
    },
    {
        id: 22,
        name: '러브리아의 편지 22',
        url:'/',
        data: '뭐? 벌써 2주년이라구? 참 시간 빠르죠? 그동안 많은 일들이 있었고 너무 힘든 일도 있었지만 그래도 고생했어요<br>' +
            '우리 럽다분들 항상 러브리아들이 응원하고 또 사랑의 눈빛으로 ( ͡° ͜ʖ ͡°)<br>' +
            '이렇게 쳐다보고 있으니까<br>' +
            '하고 싶은거 다 하시길 바랄게요<br>' +
            '언젠가 될진 모르겠지만 언젠간<br>' +
            '럽다 도쿄돔 공연 있을 때 까지 항상<br>' +
            '응원하고 또 응원할게요! 미리 축하드려요! 럽다 돔공연 축하해!',
        type: 'text',
        grade: 'R',
        unlocked: false,
        isNew: false
    },
    {
        id: 23,
        name: '러브리아의 편지 23',
        url:'/',
        data: '2주년 너무너무너무너무너무<br>' +
            '축하드립니다!!!<br>' +
            '이 험난한 버츄얼 아이돌 세계에서 2주년이라니 정말 대견한 마음이 들어요...<br>' +
            '2년동안 있던 많은 일들은 이제 웃으며 넘길 수 있도록<br>' +
            '행복한 미래를<br>' +
            '우리 러브리아와 만들어가요!<br>' +
            '사랑해요!!!!',
        type: 'text',
        grade: 'R',
        unlocked: false,
        isNew: false
    },
    {
        id: 24,
        name: '러브리아의 편지 24',
        url:'/',
        data: '1주년이 한참지나고 나서야 알게되었는데 2주년에는 함께 할 수 있어서 정말 기쁘네요<br>' +
            '하유님은 무료한 일상에 우연히 날아온 행운이었어요<br>' +
            '덕분에 러브다이아를 알게되었고<br>' +
            '힘들어도 웃을 수 있는 날들이 많아졌어요<br>' +
            '지금까지 늘 감사했고<br>' +
            '앞으로도 늘 좋은 일들만 가득하기를<br>' +
            '바라면서 방송챙겨볼게요<br>' +
            '러브다이아 화이팅!',
        type: 'text',
        grade: 'R',
        unlocked: false,
        isNew: false
    },
    {
        id: 25,
        name: '러브리아의 편지 25',
        url:'/',
        data: '러브다이아분들덕에 정신없이 재밌게 시간을 보내니 어느덧 2주년을 앞두고 있네요.<br>' +
            '항상 저희를 위해주시는 마음씨가 얼마나 고맙고 사랑스러운지 몰라요.<br>' +
            '앞으로 10년이고 100년이고 계속계속 재밌고 즐거운 나날을 보냈으면 좋겠어요.<br>' +
            '러브다이아 화이팅!!',
        type: 'text',
        grade: 'R',
        unlocked: false,
        isNew: false
    },
    {
        id: 26,
        name: '러브리아의 편지 26',
        url:'/',
        data: '생활관 동기가 저한테 그러더라구요<br>' +
            '덕질하는게 뭐라고<br>' +
            '별거 아닌걸로 그렇게 즐거워하냐고<br>' +
            '그쵸 별거 아니죠<br>' +
            '<br>' +
            '럽다는 \'별\'이 아니라<br>' +
            '<br>' +
            '우주를 꾸며주는 \'은하\'니까요<br>' +
            '<br>' +
            '이 은하를 덕질할 수 있어 행복하다<br>' +
            '통장은 아파도 나는 안아프니<br>' +
            '어쨌든 행복하다 헤헿',
        type: 'text',
        grade: 'R',
        unlocked: false,
        isNew: false
    },
    {
        id: 27,
        name: '러브리아의 편지 27',
        url:'/',
        data: '💕 2주년을 진심으로 축하해요!<br>' +
            '함께한 모든 순간들이 소중하고 감사해요.<br>' +
            '앞으로도 변치 않는 방송 기대할게요. 💕',
        type: 'text',
        grade: 'R',
        unlocked: false,
        isNew: false
    },
    {
        id: 28,
        name: '러브리아의 편지 28',
        url:'/',
        data: '2주년이라는 추억으로<br>' +
            '우리 모두 행복하니까<br>' +
            '추억이 모인 내일은 오늘보다<br>' +
            '더 행복할 거야!',
        type: 'text',
        grade: 'R',
        unlocked: false,
        isNew: false
    },
    {
        id: 29,
        name: '러브리아의 편지 29',
        url:'/',
        data: '내가 우리 럽다를 알게되고 럽며든지 벌써 2년이구나~<br>' +
            '정말 많은 일들이 있었지만 잘 버텨주어서 고맙고<br>' +
            '팬으로써 뭔가 더 많이 못해줘서 미안하고.<br>' +
            '럽다와 러브리아 우리 모두 앞으로 끝까지 쭉 함께하자!<br>' +
            '빙하유! 양메이! 유리리 사랑해! 알러뷰♡♡',
        type: 'text',
        grade: 'R',
        unlocked: false,
        isNew: false
    },
    {
        id: 30,
        name: '러브리아의 편지 30',
        url:'/',
        data: '안녕하세요 러브다이아 2주년을 진심으로 축하합니다!<br>' +
            '비록 러브다이아를 접한지는 얼마 안 됐지만<br>' +
            '앞으로는 더많은 활동 기대하면서 빙하유 양메이 유리리 화이팅입니다.<br>' +
            '항상 고마워요 방송해줘서!',
        type: 'text',
        grade: 'R',
        unlocked: false,
        isNew: false
    },
    {
        id: 31,
        name: '러브리아의 편지 31',
        url:'/',
        data: '러브다이아 앞으로도 ㅎㅇㅌ!!',
        type: 'text',
        grade: 'R',
        unlocked: false,
        isNew: false
    },
    {
        id: 32,
        name: '러브리아의 편지 32',
        url:'/',
        data: '럽하럽하! 어느새 2주년이네요! 지금까지 우리랑 함께해줘서 고마워요:)<br>' +
            '럽다 덕에 위로도 받고, 힘도 내고, 행복했고 앞으로도 그럴거예요! 그러니 앞으로도 오래 같이 행복하게 함께해주세요:D',
        type: 'text',
        grade: 'R',
        unlocked: false,
        isNew: false
    },
    {
        id: 33,
        name: '러브리아의 편지 33',
        url:'/',
        data: '러브다이아 2주년 축하드립니다<br>' +
            '앞으로 가는 길이 항상 꽃길이길<br>' +
            '바라며 항상 빙식쿤, 털찌,<br>' +
            '찌부로서 함께 걸어가고 싶습니다<br>' +
            '화이팅!',
        type: 'text',
        grade: 'R',
        unlocked: false,
        isNew: false
    },
    {
        id: 34,
        name: '러브리아의 편지 34',
        url:'/',
        data: '항상 말도 안듣고 숭한말만 하는 시청자지만 같이 놀아줘서 고마워요<br>' +
            '이제 방송이 일상에 한부분으로 자리잡았네요<br>' +
            '그 동안 좋은추억이 생겼고<br>' +
            '앞으로 만들어갈 추억들이 기대됩니다 언제나 사랑하고<br>' +
            '우리 오래오래 봅시다.',
        type: 'text',
        grade: 'R',
        unlocked: false,
        isNew: false
    },
    {
        id: 35,
        name: '러브리아의 편지 35',
        url:'/',
        data: '럽다 사랑해요우 축하해요우',
        type: 'text',
        grade: 'R',
        unlocked: false,
        isNew: false
    },
    {
        id: 36,
        name: '러브리아의 편지 36',
        url:'/',
        data: '2주년 축하해요<br> 저는 러브다이아를 안지 얼마 되지않았지만<br> 앞으로 함께할 나날들이 많으니 괜찮겠죠?<br> 앞으로도 힘내서 방송해주세요¡',
        type: 'text',
        grade: 'R',
        unlocked: false,
        isNew: false
    },
    {
        id: 37,
        name: '러브리아의 편지 37',
        url:'/',
        data: '데뷔 때 부터 지켜봐온 러브리아 입니다!!<br>' +
            '벌써 럽다가 2주년??<br>' +
            '1주년 한지 얼마 안된 것 같은데, 벌써 2주년 이라니!!<br>' +
            '2주년 까지 많은 일이 많았지만, 다들 힘내주시고 열심히 해주시는 모습 보기 좋았어요!!<br>' +
            '3주년, 4주년.. 그 이상까지 계속 방송해 주실꺼죠?<br>' +
            '2주년 너무너무 축하드리고, 앞으로도 화이팅!!<br>' +
            '재밌는 콘텐츠들 기대하고 있겠습니다!!<br>' +
            '사랑한다 러브다이아!!!',
        type: 'text',
        grade: 'R',
        unlocked: false,
        isNew: false
    },
    {
        id: 38,
        name: '러브리아의 편지 38',
        url:'/',
        data: '2주년 너무너무 축하드립니다!<br>' +
            '러브다이아가 2주년이 될 동안<br>' +
            '학생이던 제가 사회인이 되었어요<br>' +
            '<br>' +
            '저도 럽다도 짧은 기간동안 많은게 바뀌었어요<br>' +
            '하지만 럽다를 좋아하고 응원하는 마음만은 변하지 않았네요!<br>' +
            '<br>' +
            '이전에도 그랬고 앞으로도<br>' +
            '러브리아로서 사랑할게요<br>' +
            '우리 오래 만나요<br>' +
            '❤️',
        type: 'text',
        grade: 'R',
        unlocked: false,
        isNew: false
    },
    {
        id: 39,
        name: '러브리아의 편지 39',
        url:'/',
        data: '벌써 2주년!<br>' +
            '입덕하고서 러브다이아와 함께한 하루하루가 즐거웠어요.<br>' +
            '즐거울 때도, 때론 슬플 때도 러브리아와 함께해준 하유님, 메이님, 리리님 모두 고마웠습니다.<br>' +
            '앞으로 더 빛날 우리의 아이돌!<br>' +
            '러브다이아를 계속해서 응원할게요!',
        type: 'text',
        grade: 'R',
        unlocked: false,
        isNew: false
    },
    {
        id: 40,
        name: '러브리아의 편지 40',
        url:'/',
        data: '러브다이아를 접하고 러브리아가 된게 벌써 일년 반 전이네요<br>' +
            '다들 제가 처음 봤던 첫 이상 그대로이신 것 같아요<br>' +
            '리리님은 앙큼한 도도한 고양이 느낌이었고<br>' +
            '하유님은 청초인줄 알았는데..매콤이었어서 기억에 깊게 남네요<br>' +
            '메이님은 진짜 엄청난 텐션이라 친구 같아서 가장 편했어요<br>' +
            '러브다이아를 보면서 트위치를 처음 접하게 되었고 지금도 러브다이아만 보고있습니다<br>' +
            '항상 응원하고 앞으로도 재밌게 같이 놀며 즐거운 방송을 이어나가주셨으면 좋겠어요<br>' +
            '2주년 축하드립니다',
        type: 'text',
        grade: 'R',
        unlocked: false,
        isNew: false
    },
    {
        id: 41,
        name: '러브리아의 편지 41',
        url:'/',
        data: '2주년 너무 축하드립니다!!<br>' +
            '<br>' +
            '앞으로도 열심히 응원할테니<br>' +
            '<br>' +
            '늘 즐겁게 언제나 웃으며<br>' +
            '<br>' +
            '방송하시길 바랍니다~',
        type: 'text',
        grade: 'R',
        unlocked: false,
        isNew: false
    },
    {
        id: 42,
        name: '러브리아의 편지 42',
        url:'/',
        data: '러브다이아를 알게 된지는 오래 되지 않았지만 2주년 진심으로 축하드립니다!!!<br>' +
            '저의 일상에 가장 힘들때 찾아와 가장 밝게 빛나는 별이 되어 주셔서 감사해요!!!<br>' +
            '이제는 러브다이아가 없는 삶은 상상이 안가요. 그러니까 우리 10년 20년 30년 40년 50년 쭉 계속 같이 있어요.<br>' +
            '<br>' +
            '이렇게 말하지만 슬프게도 어느 순간 헤어지는 순간이 오겠죠... 헤어져도 저의 영원한 별이에요. 같이 함께한 추억을 아름답게 고이고이 간직하고 있을게요. 항상 아름답고 마음이 따뜻하신 러브다이아 항상 사랑해요!<br>' +
            '<br>' +
            'ㅂㅎㅇ ㄱㅇㅇ',
        type: 'text',
        grade: 'R',
        unlocked: false,
        isNew: false
    },
    {
        id: 43,
        name: '러브리아의 편지 43',
        url:'/',
        data: '1주년 축하하고 얼마 안 지난 것 같은데 어느새 2주년이 찾아왔네요!!<br>어떤 모습이든 열심히 응원할게요!!',
        type: 'text',
        grade: 'R',
        unlocked: false,
        isNew: false
    },
    {
        id: 44,
        name: '러브리아의 편지 44',
        url:'/',
        data: '러브다이아의 2주년을 진심으로 축하드립니다!!!<br>' +
            '제일 좋아하는 그룹이 어느새 2주년이 되었네요.<br>' +
            '러브다이아가 시작하는 모습을 봐서 그런지 유난히 더 애정이 갑니다.<br>' +
            '우리 러브다이아 앞으로도 쭈욱 롱런하고 행복했으면 좋겠습니다.<br>' +
            '<br>' +
            '러브다이아! 사랑해!!!',
        type: 'text',
        grade: 'R',
        unlocked: false,
        isNew: false
    },
    {
        id: 45,
        name: '러브리아의 편지 45',
        url:'/',
        data: '데뷔방송 하신게 얼마전 같은데 벌서 1주년을 지나 2주년이 돌아왔네요!<br>' +
            '<br>' +
            '앞으로도 러브리아들과 함께 좋은날 보내며 행복하게 3주년, 5주년, 10년까지 기원합니다!',
        type: 'text',
        grade: 'R',
        unlocked: false,
        isNew: false
    },
]

// SR 등급 오브젝트
const item_sr = [
    {
        id: 1,
        name: '컨텐츠 작업자의 편지 1',
        url:'/',
        data: '러브다이아 2주년 축하해요~!!!!!!!<br>' +
            '<br>' +
            '하루하루 너무 즐겁게 보고있습니다!<br>' +
            '<br>' +
            '기획자 닉언밴님께서 사이트를 하나 만들어보자고 하셔서<br>' +
            '이렇게 컨텐츠 제작에 웹 개발자로써 참여하게 되었네요!<br>' +
            '<br>' +
            '그런데 제가 주로 백엔드나 서버, 프론트 스크립트를 주로 다루는 사람인데 반해<br>' +
            '이번 컨텐츠에서 가장 중요한 점이 웹디자인과 CSS인 점에서<br>' +
            '사실 굉장히 어려운 작업이었습니다.<br>' +
            '<br>' +
            '하지만 하나씩 찾아가면서 모양을 만들어가는 게<br>' +
            '마치 러브다이아가 처음 데뷔를 하고<br>' +
            '시간이 지나면서 지금의 모습으로 만들어지는 과정처럼 느껴져서<br>' +
            '굉장히 즐거운 작업시간 이었습니다.<br>' +
            '<br>' +
            '앞으로도 좋은 방송 좋은 시간 보냈으면 좋겠습니다.<br>' +
            '<br>' +
            '러브다이아 여러분과 러브리아 모두 화이팅입니다!',
        type: 'text',
        grade: 'SR',
        unlocked: false,
        isNew: false
    },
    {
        id: 2,
        name: '컨텐츠 작업자의 편지 2',
        url:'/',
        data: '헬로 러브다이아.<br>' +
            '2주년을 맞이한걸 정말 축하해요!<br>' +
            '지난 1년 원했던 모든걸 이루셨기를 바라며<br>' +
            '내년 1년도 승승장구하는 러브다이아가 되기를!<br>' +
            '러브다이아 항상 아끼고 사랑해요~<br>' +
            '80주년 90주년까지도 함께할거에요..!<br>' +
            '<br>' +
            '모두 건강히 오래오래 봐요!!!',
        type: 'text',
        grade: 'SR',
        unlocked: false,
        isNew: false
    },
    {
        id: 3,
        name: '컨텐츠 작업자의 편지 3',
        url:'/',
        data: '러브다이아 2주년을 진심으로 축하드립니다!<br>' +
            '<br>' +
            '힘든 일들이 있었을 때 럽다가 정말 많은 힘이 돼주었던 거 같아요.<br>' +
            '<br>' +
            '항상 감사하고 앞으로도 좋은 일 가득 함께 할 수 있으면 좋겠어요:)',
        type: 'text',
        grade: 'SR',
        unlocked: false,
        isNew: false
    },
    {
        id: 4,
        name: '컨텐츠 기획자의 편지',
        url:'/',
        data: '럽다의 2주년을 축하드려요!!~🎉🎉<br>' +
            '<br>' +
            '언제나 웃음주시고 항상 힘이 되어주시는 럽다 여러분들을 위해,<br>' +
            '조금이라도 보답드리고자 1주년에 이어,이렇게 2주년에도 축하드리기위해 기획을 하게되었네요 :)<br>' +
            '<br>' +
            '부디, 럽다 여러분들을 위해 전달드리는 저희들의 편지를 받고 힘이 되었으면 좋겠어요 ♡⸜(ˆᗜˆ)<br>' +
            '항상 하고싶으신일들 모두 잘 되길 바라며, 노력하시는 럽다 여러분들을 응원합니다!<br>' +
            '앞으로도 아프지않고 오래오래 함께하길 바래요~<br>' +
            '<br>' +
            '다시 한 번 러브다이아의 2주년을 진심으로 축하합니다!!!<br>' +
            '럽다 뽀에버 ♡⸜(˶˃ ᵕ ˂˶)⸝♡ 사랑합니다~!!',
        type: 'text',
        grade: 'SR',
        unlocked: false,
        isNew: false
    },
    {
        id: 5,
        name: '컨텐츠 작업자의 편지 4',
        url:'/',
        data: '러브다이아 2주년을 축하해요! 🎉<br>' +
            '1년 동안 응원해온 팬으로서 러브다이아의 성장과 함께한 시간들이 정말 소중했을 것 같아요.<br>' +
            '멤버들의 매력 넘치는 모습과 멋진 퍼포먼스 덕분에 매일이 행복했죠.<br>' +
            '앞으로도 러브다이아가 계속해서 반짝이는 순간들을 만들어가길 바라며,<br>' +
            '언제나 든든한 팬으로 응원할게요!<br>' +
            '2주년 축하해요, 더 많은 추억 쌓아가요! 💖',
        type: 'text',
        grade: 'SR',
        unlocked: false,
        isNew: false
    },
    {
        id: 6,
        name: '컨텐츠 작업자의 편지 5',
        url:'/',
        data: '러브다이아의 2주년을 축하합니다!<br>' +
            '<br>' +
            '벌써 2주년이라니..<br>' +
            '최근에는 자주 못봤지만<br>' +
            '항상 뒤에서 응원하고 있습니다<br>' +
            '할 말은 많은데<br>' +
            '막상 적으려니 부끄럽네요<br>' +
            '그러므로 이만!<br>' +
            '<br>' +
            '럽다 화이팅!',
        type: 'text',
        grade: 'SR',
        unlocked: false,
        isNew: false
    },
    {
        id: 7,
        name: '컨텐츠 작업자의 작품 1',
        url:'/',
        data: '<img src="/asset/img/럽다%202주년%20뽑기%203인.png" alt="" style="width: 100%">',
        type: 'img',
        grade: 'SR',
        unlocked: false,
        isNew: false
    },
    {
        id: 8,
        name: '컨텐츠 작업자의 작품 2',
        url:'/',
        data: '<img src="/asset/img/2주년%20메인%201920.png" alt="" style="width: 100%">',
        type: 'img',
        grade: 'SR',
        unlocked: false,
        isNew: false
    },
    {
        id: 9,
        name: '컨텐츠 작업자의 작품 3',
        url:'/',
        data: '<img src="/asset/img/2주년%20메인%201920%202.png" alt="" style="width: 100%">',
        type: 'img',
        grade: 'SR',
        unlocked: false,
        isNew: false
    },
    {
        id: 10,
        name: '어느 충직한 동부의 편지 1',
        url:'/',
        data: '양메이! 양메이! 양메이!<br>' +
            '우윳빛갈양메이! 사랑해요 양메이!<br>' +
            '언제나 오래오래 영원히! 우리곁에있어주세요!',
        type: 'text',
        grade: 'SR',
        unlocked: false,
        isNew: false
    },
    {
        id: 11,
        name: '하우스켓 님의 편지',
        url:'/',
        data: '러브다이아가 2주년 너무 축하드려요!!<br>' +
            '러브다이아는 제 첫 버츄얼 외주 작업이었답니다.<br>' +
            '<br>' +
            '저는 정말 행운아라고 생각해요 <br>' +
            '인지도하나 없이 무명으로 커미션 작업하던 사람을 <br>' +
            '이만큼 성장시켜 끌어올려준건 러브다이아였어요.<br>' +
            '<br>' +
            '아마 러브다이아가 없었다면 지금의 저도 없었을거라고 저는 단언합니다 🙂 <br>' +
            '<br>' +
            '항상항상 너무 감사한 마음을 품고있습니다. <br>' +
            '앞으로도 좋은 활동 부탁드리고 방송 정말 재밌게 보고있습니다!',
        type: 'text',
        grade: 'SR',
        unlocked: false,
        isNew: false
    },
    {
        id: 12,
        name: '컨텐츠 작업자의 작품 4',
        url:'/',
        data: '<img src="/asset/img/sr%20hayu.png" alt="" style="width: 100%; height: auto">',
        type: 'img',
        grade: 'SR',
        unlocked: false,
        isNew: false
    },
    {
        id: 13,
        name: '컨텐츠 작업자의 작품 5',
        url:'/',
        data: '<img src="/asset/img/sr%20lili.png" alt="" style="width: 100%; height: auto">',
        type: 'img',
        grade: 'SR',
        unlocked: false,
        isNew: false
    },
    {
        id: 14,
        name: '컨텐츠 작업자의 작품 5',
        url:'/',
        data: '<img src="/asset/img/sr%20meimei.png" alt="" style="width: 100%; height: auto">',
        type: 'img',
        grade: 'SR',
        unlocked: false,
        isNew: false
    },
]

// SSR 등급 오브젝트
const item_ssr = [
    {
        id: 1,
        name: '<span style="color: #ff9c00;">수퍼 초인 샤벨 토냐님의 편지</span>',
        url:'',
        data: '<img src="/asset/img/0403dac3e6b6dac9.png" style="opacity: 0.2; width: 400px; position: absolute; z-index: -1">' +
            '<span style="">하유님 메이님 리리님! 오랜만이에요!!!!<br>' +
            '벌써 럽다도 2주년이 되었네요!<br>' +
            '시간 참 빠르죠 ㅋㅋㅋ 저도 벌써 3주년이 다가오고 있답니다....<br>' +
            '그간 플랫폼 문제도 있었고 아쉽게도 이제는 더 이상 5명이 있는 럽다가 아니게 되었지만<br>' +
            '항상 너머에서 응원하고 있었어요!<br>' +
            '직접적으로 도움드릴 수 있는 일이 있었다면 참 좋았겠지만<br>' +
            '그래도 잘 헤쳐나가신 것 같아서 너무 다행입니다 히히<br>' +
            '앞으로도 계속 오래오래 함께 활동할 수 있었으면 좋겠어요!<br>' +
            '<span style="font-weight: bold">졸업절대막아절대지켜!!!!!!</span><br>' +
            '요즘 너무 바빠서 자주 연락도 못드리고 했는데 조만간 또 같이 게임하거나 밥먹으러 가요!!!!<br>' +
            '앞으로의 럽다도 즐겁고 행복한 일만 가득하길 진심으로 바랍니다 사랑해요!!!!!</span>',
        type: 'text',
        grade: 'SSR',
        unlocked: false,
        isNew: false
    },
    {
        id: 2,
        name: '반님의 영상편지',
        url:'https://drive.google.com/file/d/1b5dFYvR8h69cLW_2TWLaFG2qUm7V58Od/preview',
        data: '스타데이즈 반님의 영상편지입니다.',
        type: 'video',
        grade: 'SSR',
        unlocked: false,
        isNew: false
    },
    {
        id: 3,
        name: '테리님의 영상편지',
        url:'https://drive.google.com/file/d/1TPRrk9Egl3SVwf466iqDz_Ql5NXTFtBr/preview',
        data: '스타데이즈 테리님의 영상편지입니다.',
        type: 'video',
        grade: 'SSR',
        unlocked: false,
        isNew: false
    },
    {
        id: 4,
        name: '나츠키님의 영상편지',
        url:'https://drive.google.com/file/d/12LPcWhamV1uu095Hk5rvVTB78_tMQKpG/preview',
        data: '스타데이즈 나츠키님의 영상편지입니다.',
        type: 'video',
        grade: 'SSR',
        unlocked: false,
        isNew: false
    },
    {
        id: 5,
        name: '류채경님의 영상편지',
        url:'https://drive.google.com/file/d/1YGykLzEyirjV6q0_dkWKX9mnJAWiH2gq/preview',
        data: '스타게이저의 류채경님의 영상편지입니다.',
        type: 'video',
        grade: 'SSR',
        unlocked: false,
        isNew: false
    },
]

export {
    item_r,
    item_sr,
    item_ssr
}